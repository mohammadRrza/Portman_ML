...
class GetPortsStatus(BaseCommand):
    ...
    def run_command(self, protocol=None):
        self.tn.write("line\r\n".encode('utf-8'))
        ...
        for slot in range(1, self.slots + 1):
            ...
            self.tn.write("\r\n".encode('utf-8'))
            time.sleep(20)
            data.append(self.tn.read_until('LINE >', 40))
        access_port_info = {}
        for record in re.compile('----------Shelf 0 ').split('\n'.join(data)):
            lines = record.split('\n\r')
            ...
            try:
                ...
                port_info['PORT_INDEX'] = '{0}{1:02}'.format(slot_number,  int(port_number))
        ...
        data = self.tn.read_until('end_of_line', 10)
        lines = re.compile("-----------------SHELF\sNUM\s0--CARD\sNUM\s").split(data)
        for line in lines:
            ...
            slot_number = re.search("(\d+)---------------------------------------", line).groups()[0]
            rows = [m.groupdict() for m in com.finditer(line)]
            for row in rows:
                try:
                    port_index = '{0}{1:02}'.format(slot_number, int(row['port_number']))
                    access_port_info[port_index]['LINE_PROFILE'] = row['LINE_PROFILE']
                    if str(row['PORT_ADMIN_STATUS']) == 'OPEN':
                        access_port_info[port_index]['PORT_ADMIN_STATUS'] = 'UNLOCK'
                    else:
                        access_port_info[port_index]['PORT_ADMIN_STATUS'] = 'LOCK'
                except Exception as ex:
                    continue
