...
class BaseCommand:
    ...
    def enable_config_mode(self):
        ...
        if self.tn.read_until(b'Password:'):
            self.tn.write((self.__telnet_password + "\r\n").encode('utf-8'))

        self.tn.write(b"config\r\n")
        active = self.tn.read_until(b"(config)#", 3)
        print(str(active), "#####")
    ...
    def get_gpon_command(self, slotNumber = None):
        ...
        if len(str(slot_number)) == 0 or slot_number == "" or slot_number == " ":
            slot_number = "1"
