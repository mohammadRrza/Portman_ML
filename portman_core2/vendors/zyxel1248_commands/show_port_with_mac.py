            ...
            if self.__mac in result:
                ...
                for inx, val in enumerate(result):
                    if self.__mac in val:
                        port = result[inx - 3]
                        if 'Port:' in port and 'Enet1' not in port:
                            return dict(result=f"card: 1, {port.strip()}", status=200)
                        else:
                            return dict(result="The mac there is in the port: Enet1", status=500)
            else:
