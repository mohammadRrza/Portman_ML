    ...
    def run_command(self):
        try:
            ...
            tn.write("port adsl annexm disable {0}-{1}\r\n".format(self.__port_indexes['slot_number'],
                                                                   self.__port_indexes['port_number']).encode('utf-8'))
            time.sleep(0.5)
            tn.write(b"exit\r\n")
            tn.write(b"y\r\n")
            tn.close()
            ...
            print('******************************************')
            return dict(result=f"ports adsl annexm disable slot {self.__port_indexes['slot_number']} port {self.__port_indexes['port_number']}",
