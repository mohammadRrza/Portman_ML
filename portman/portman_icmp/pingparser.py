...
def parse(ping_output):
    ...
    try:
        ...
        com = re.compile(r'(?P<transmitted>(\d{1,10})\s(packets transmitted)),\s(?P<received>(\d{1,10})\s(received)),\s\+\d{1,10}\s(errors|duplicates),\s(?P<loss>(\d{1,10}%)\s(packet loss)),\s(?P<time>time\s(\d{1,10})ms)',re.MULTILINE | re.I)
        received = com.search(ping_output).group('received').split()[0]
        sent = com.search(ping_output).group('transmitted').split()[0]
        packet_loss = com.search(ping_output).group('loss').split()[0].split('%')[0]
        time = com.search(ping_output).group('time').split()[1]
