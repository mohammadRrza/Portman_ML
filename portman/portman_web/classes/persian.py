...
def unidecode(input_str):
    final_str = convert_ar_numbers(input_str)
    final_str = convert_fa_numbers(final_str)
    return final_str


def checkIsFloat(element):
    if element is None: 
        return False
    try:
        float(element)
...
def reFormatLatLong(item: str):
    return item.replace("°", "-").replace("'", "-").replace("\"", "").replace(" ","")

def standard_lat(latitude):
    latitude = reFormatLatLong(latitude)
    if (checkIsFloat(latitude)):
        return latitude
    return sum(float(x) / 60 ** n for n, x in enumerate(latitude[:-1].split('-')))  * (1 if 'N' in latitude[-1] else -1)

def standard_long(longitude):
    longitude = reFormatLatLong(longitude)
    if (checkIsFloat(longitude)):
        return longitude
    return sum(float(x) / 60 ** n for n, x in enumerate(longitude[:-1].split('-'))) * (1 if 'E' in longitude[-1] else -1)