...
class Shutdown(BaseCommand):
    def __init__(self, params):
        ...
        self.__SSH_password = 'itIT' # params.get('SSH_password')
    ...
    def run_command(self):
        try:
            ...
            net_connect.disconnect()
            return dict(result="Successfull. Interface: " + interface, output=output, status=200)
        except Exception as ex:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
            return dict(result="Error: " + str(ex) + "  / Line: " + str(exc_tb.tb_lineno), status=500)
