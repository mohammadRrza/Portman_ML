...
class BaseCommand:
    def __init__(self, params):
        ...
        self.regex_search_pattern = r'.*'
        self.regex_sub_pattern = r"\b[\w\s]+\b"
    ...
    def login(self):
        ...
        time.sleep(1)
        err = tn.read_very_eager() #tn.read_until(b'end*', 3)
        ...
        self.end_string = "telnet@" + str(err).split("@")[1].split(">")[0].replace(" end'", "").strip() + ">"
        return True
    ...
    def fetch_command_output(self):
        ...
        self.tn.write((self.command_str + "\r\n\x15").encode('utf-8'))
    ...
    def clean_output(self, result):
        ...
        def _gc(item):
            for gc_text in self.garbage_list:
                item = item.replace(gc_text, "")
            return item


        result = [_gc(val) for val in result]
    ...
    def fetch_output_errors(self):
        ...
        if 'Unknown command,' in self.output or 'Bad command' in self.output:
