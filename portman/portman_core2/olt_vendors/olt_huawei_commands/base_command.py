...
class BaseCommand:
    ...
    def get_gpon_command(self, slotNumber = None):
        ...
        return "interface gpon 0/{0}".format(slot_number)
    ...
    def enable_gpon_mode(self):
        ...
        if "Parameter error" in str(result):
            return dict(result="Slot number is wrong.", status=500)
        time.sleep(1)
        return True
    ...
    def extractModems(self, string, snEndsWith = ''):
        modems = []
        rows = string.split('----------------------------------------------------------------------------')
        for row in rows: 
            fs_port_match = re.search(r'F/S/P\s*:\s*(\d+/\d+/\d+)', row)
            ont_sn_match = re.search(r'Ont SN\s*:\s*(\d+)\s*\((\w+-\d+)\)', row)
            equipment_id_match = re.search(r'Ont EquipmentID\s*:\s*(\w+)', row)
            if ont_sn_match is None:
                ont_sn_match = re.search(r'Ont SN\s+:\s+(\S+)', row)

            fs_port, ont_sn, equipment_id = None, None, None

            if ont_sn_match and ont_sn_match.group(1).endswith(snEndsWith):
                ont_sn = ont_sn_match.group(1)
                #ont_sn_label = ont_sn_match.group(2)
                fs_port = fs_port_match.group(1)
                equipment_id = equipment_id_match.group(1)
                modems.append(dict(slot_port=fs_port.split('/'), serial=ont_sn, equipment_id=equipment_id))
