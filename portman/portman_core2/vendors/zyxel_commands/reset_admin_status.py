...
class ResetAdminStatus(BaseCommand):
    ...
    def run_command(self):
        ...
        for port_item in self.__port_indexes:
            lock_target_oids.append(
                ('.{0}.{1}'.format(self.PORT_ADMIN_STATUS_OID, port_item['port_index']), rfc1902.Integer(lock)))
            unlock_target_oids.append(
                ('.{0}.{1}'.format(self.PORT_ADMIN_STATUS_OID, port_item['port_index']), rfc1902.Integer(unlock)))
